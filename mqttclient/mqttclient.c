...
static void mqtt_packet_drain(mqtt_client_t *c, platform_timer_t *timer,
                              int packet_len) {
...
static int mqtt_read_packet(mqtt_client_t *c, int *packet_type,
...
static int mqtt_send_packet(mqtt_client_t *c, int length,
                            platform_timer_t *timer) {
  int len = 0;
  int sent = 0;
...
static int mqtt_deliver_message(mqtt_client_t *c, MQTTString *topic_name,
                                mqtt_message_t *message) {
...
static ack_handlers_t *mqtt_ack_handler_create(mqtt_client_t *c, int type,
                                               uint16_t packet_id,
                                               uint16_t payload_len,
                                               message_handlers_t *handler) {
  ack_handlers_t *ack_handler = NULL;

  ack_handler = (ack_handlers_t *)platform_memory_alloc(sizeof(ack_handlers_t) +
                                                        payload_len);
  if (NULL == ack_handler)
    return NULL;
  ...
  platform_timer_cutdown(&ack_handler->timer,
                         c->mqtt_cmd_timeout); /* No response within timeout
                                                  will be destroyed or resent */
...
static void mqtt_ack_handler_destroy(ack_handlers_t *ack_handler) {
  if (NULL != &ack_handler->list) {
    ...
    platform_memory_free(
        ack_handler); /* delete ack handler from the list, and free memory */
  }
}
...
static int mqtt_ack_list_node_is_exist(mqtt_client_t *c, int type,
                                       uint16_t packet_id) {
  mqtt_list_t *curr, *next;
  ack_handlers_t *ack_handler;
  ...
  LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_ack_handler_list) {
    ...
    /* For mqtt packets of qos1 and qos2, you can use the packet id and type as
       the unique identifier to determine whether the node already exists and
...
static int mqtt_ack_list_unrecord(mqtt_client_t *c, int type,
                                  uint16_t packet_id,
                                  message_handlers_t **handler) {
  mqtt_list_t *curr, *next;
  ack_handlers_t *ack_handler;
...
static int mqtt_msg_handler_is_exist(mqtt_client_t *c,
  ...
  LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
    ...
    /* determine whether a node already exists by mqtt topic, but wildcards are
     * not supported */
    if ((NULL != msg_handler->topic_filter) &&
        (mqtt_is_topic_equals(msg_handler->topic_filter,
                              handler->topic_filter))) {
      MQTT_LOG_W("%s:%d %s()...msg_handler->topic_filter: %s, "
...
static void mqtt_ack_list_scan(mqtt_client_t *c, uint8_t flag) {
  ...
  LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_ack_handler_list) {
    ...
    if ((ack_handler->type == PUBACK) || (ack_handler->type == PUBREC) ||
      ...
      /* timeout has occurred. for qos1 and qos2 packets, you need to resend
       * them. */
...
static int mqtt_try_resubscribe(mqtt_client_t *c) {
  ...
  LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
    ...
    if ((rc = mqtt_subscribe(c, msg_handler->topic_filter, msg_handler->qos,
      ...
      MQTT_LOG_W("%s:%d %s()... mqtt ack handler num too much ...", __FILE__,
...
static int mqtt_try_reconnect(mqtt_client_t *c) {
  ...
  if (MQTT_SUCCESS_ERROR != rc) {
    /*connect fail must delay reconnect try duration time and let cpu time go
     * out, the lowest priority task can run */
...
static int mqtt_publish_ack_packet(mqtt_client_t *c, uint16_t packet_id,
                                   int packet_type) {
  int len = 0;
  int rc = MQTT_SUCCESS_ERROR;
  platform_timer_t timer;
  ...
  switch (packet_type) {
  case PUBREC:
    len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBREL,
                            0, packet_id); /* make a PUBREL ack packet */
  ...
  case PUBREL:
    len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBCOMP,
                            0, packet_id); /* make a PUBCOMP ack packet */
    break;
...
static int mqtt_publish_packet_handle(mqtt_client_t *c,
  ...
  if (msg.qos != QOS0) {
    ...
    if (msg.qos == QOS1)
      len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBACK,
                              0, msg.id);
    else if (msg.qos == QOS2)
      len = MQTTSerialize_ack(c->mqtt_write_buf, c->mqtt_write_buf_size, PUBREC,
                              0, msg.id);
  ...
  if (msg.qos != QOS2)
    ...
    /* record the received of a qos2 message and only processes it when the qos2
...
static int mqtt_pubrec_and_pubrel_packet_handle(mqtt_client_t *c,
  ...
  (void)dup;
  rc = mqtt_publish_ack_packet(c, packet_id,
                               packet_type); /* make a ack packet and send it */
...
static int mqtt_packet_handle(mqtt_client_t *c, platform_timer_t *timer) {
  ...
  case 0: /* timed out reading packet or an error occurred while reading data*/
    if (MQTT_BUFFER_TOO_SHORT_ERROR == rc) {
      MQTT_LOG_E("the client read buffer is too short, please call "
                 "mqtt_set_read_buf_size() to reset the buffer size");
      /* don't return directly, you need to stay active, because there is data
       * readable now, but the buffer is too small */
    }
    break;
...
static int mqtt_wait_packet(mqtt_client_t *c, int packet_type,
                            platform_timer_t *timer) {
  int rc = MQTT_FAILED_ERROR;
...
static void mqtt_yield_thread(void *arg) {
  ...
  if (CLIENT_STATE_CONNECTED != state) {
    ...
    platform_thread_stop(
        c->mqtt_thread); /* mqtt is not connected to the server, stop thread */
  }

  while (1) {
    rc = mqtt_yield(c, c->mqtt_cmd_timeout);
    if (MQTT_CLEAN_SESSION_ERROR == rc) {
      MQTT_LOG_W("%s:%d %s()..., mqtt clean session....", __FILE__, __LINE__,
                 __FUNCTION__);
  ...
  if (rc == MQTT_SUCCESS_ERROR) {
    if (NULL == c->mqtt_thread) {
      ...
      c->mqtt_thread = platform_thread_init(
          "mqtt_yield_thread", mqtt_yield_thread, c, MQTT_THREAD_STACK_SIZE,
          MQTT_THREAD_PRIO, MQTT_THREAD_TICK);
      ...
      if (NULL != c->mqtt_thread) {
        ...
        network_release(c->mqtt_network);
        rc = MQTT_CONNECT_FAILED_ERROR;
        MQTT_LOG_W("%s:%d %s()... mqtt yield thread creat faile...", __FILE__,
                   __LINE__, __FUNCTION__);
      }
    } else {
      mqtt_set_client_state(c,
                            CLIENT_STATE_CONNECTED); /* reconnect, mqtt thread
                                                        is already exists */
    }
...
static uint32_t mqtt_read_buf_malloc(mqtt_client_t *c, uint32_t size) {
  ...
  if (NULL == c->mqtt_read_buf) {
    MQTT_LOG_E("%s:%d %s()... malloc read buf failed...", __FILE__, __LINE__,
               __FUNCTION__);
    RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
  }
...
static uint32_t mqtt_write_buf_malloc(mqtt_client_t *c, uint32_t size) {
  ...
  if (NULL == c->mqtt_write_buf) {
    MQTT_LOG_E("%s:%d %s()... malloc write buf failed...", __FILE__, __LINE__,
               __FUNCTION__);
    RETURN_ERROR(MQTT_MEM_NOT_ENOUGH_ERROR);
  }
...
int mqtt_keep_alive(mqtt_client_t *c) {
  ...
  if (platform_timer_is_expired(&c->mqtt_last_sent) ||
    ...
    if (c->mqtt_ping_outstanding) {
      ...
      platform_timer_t timer;
      int len =
          MQTTSerialize_pingreq(c->mqtt_write_buf, c->mqtt_write_buf_size);
...
int mqtt_release(mqtt_client_t *c) {
  ...
  while ((CLIENT_STATE_INVALID != mqtt_get_client_state(c))) {
    ...
    // optimization.
    if (platform_timer_is_expired(&timer)) {
      MQTT_LOG_E("%s:%d %s()... mqtt release failed...", __FILE__, __LINE__,
                 __FUNCTION__);
      RETURN_ERROR(MQTT_FAILED_ERROR)
    }
  }
...
int mqtt_subscribe(mqtt_client_t *c, const char *topic_filter, mqtt_qos_t qos,
  ...
  if (NULL == handler)
    handler = default_msg_handler; /* if handler is not specified, the default
                                      handler is used */
...
int mqtt_unsubscribe(mqtt_client_t *c, const char *topic_filter) {
  ...
  if ((len =
           MQTTSerialize_unsubscribe(c->mqtt_write_buf, c->mqtt_write_buf_size,
                                     0, packet_id, 1, &topic)) <= 0)
    goto exit;
  if ((rc = mqtt_send_packet(c, len, &timer)) != MQTT_SUCCESS_ERROR)
    goto exit;
...
int mqtt_publish(mqtt_client_t *c, const char *topic_filter,
                 mqtt_message_t *msg) {
  int len = 0;
  int rc = MQTT_FAILED_ERROR;
  platform_timer_t timer;
  ...
  if (QOS0 != msg->qos) {
    if (mqtt_ack_handler_is_maximum(c)) {
      rc = MQTT_ACK_HANDLER_NUM_TOO_MUCH_ERROR; /* the recorded ack handler has
                                                   reached the maximum */
      goto exit;
    }
...
int mqtt_list_subscribe_topic(mqtt_client_t *c) {
  ...
  LIST_FOR_EACH_SAFE(curr, next, &c->mqtt_msg_handler_list) {
     ...
     * not supported */
    if (NULL != msg_handler->topic_filter) {
      MQTT_LOG_I("%s:%d %s()...[%d] subscribe topic: %s", __FILE__, __LINE__,
                 __FUNCTION__, ++i, msg_handler->topic_filter);
    }
  }
...
int mqtt_set_will_options(mqtt_client_t *c, char *topic, mqtt_qos_t qos,
                          uint8_t retained, char *message) {
